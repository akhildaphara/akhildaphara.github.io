---
role: "Software Engineer"
company: "Genesys, USA"
date: "May 2023 — Present"
color: "lime"
tags: ["TypeScript", "Node.js", "AWS", "Java", "MCP"]
order: 1
bullets:
  - "Part of <span class=\"text-white\">Cursor AI</span> pilot to increase team productivity, successfully scaling the tool to 100% of the organization."
  - "Won internal hackathon prize for building a <span class=\"text-white\">Model Context Protocol (MCP) server</span> that integrated public APIs with IDEs and Amazon Bedrock."
  - "Designed and implemented <span class=\"text-white\">AWS Step Function workflow</span> for distributed computing, reducing compute time from hours to under 18 minutes."
  - "Developed a core <span class=\"text-white\">Node.js utility library</span> adopted by multiple teams to standardize external integrations (HTTP, Database, S3)."
---
